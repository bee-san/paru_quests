...
```bash
bash tools/encrypt_shared_pack.sh /tmp/plain-pack.json app/src/main/assets/shared-packs/current.encrypted.json bedroom-dust-bandits 6
```

The script reads `agent-skills/paruchan-shared-packs/.env`, which is ignored by git. Commit only the encrypted `current.encrypted.json` asset, never plaintext private packs.
